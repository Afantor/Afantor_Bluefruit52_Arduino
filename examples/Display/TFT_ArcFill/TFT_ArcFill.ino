...
void loop() {
  ...
  fillArc(120, 68, inc * 6, 1, 100, 60, 10, rainbow(col));

  // Continuous segmented (inc*2) elliptical arc drawing
  fillArc(120, 68, ((inc * 2) % 60) * 6, 1, 100, 60, 30, rainbow(col));

  // Circle drawing using arc with arc width = radius
  fillArc(120, 68, inc * 6, 1, 30, 30, 42, rainbow(col));
...
int fillArc(int x, int y, int start_angle, int seg_count, int rx, int ry, int w, unsigned int colour)
{
  ...
  for (int i = start_angle; i < start_angle + seg * seg_count; i += inc) {
    ...
    BF52.Lcd.fillTriangle(x0, y0, x1, y1, x2, y2, colour);
    BF52.Lcd.fillTriangle(x1, y1, x2, y2, x3, y3, colour);
