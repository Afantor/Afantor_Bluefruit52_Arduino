...
void loop() {
  ...
  fillSegment(120, 67, 0, 60, 64, RED);
  fillSegment(120, 67, 60, 30, 64, GREEN);
  fillSegment(120, 67, 60 + 30, 120, 64, BLUE);
  fillSegment(120, 67, 60 + 30 + 120, 150, 64, YELLOW);

  delay(4000);

  // Erase old chart with 360 degree black plot
  fillSegment(120, 67, 0, 360, 64, BLACK);
}
...
int fillSegment(int x, int y, int start_angle, int sub_angle, int r, unsigned int colour)
{
  ...
  for (int i = start_angle; i < start_angle + sub_angle; i++) {
    ...
    BF52.Lcd.fillTriangle(x1, y1, x2, y2, x, y, colour);
