#ifndef _AFANTOR_GFX_H
#define _AFANTOR_GFX_H
...
/// A generic graphics superclass that can handle all sorts of drawing. At a minimum you can subclass and provide drawPixel(). At a maximum you can do a ton of overriding to optimize. Used for any/all Afantor displays!
class Afantor_GFX : public Print {

 public:

  Afantor_GFX(int16_t w, int16_t h); // Constructor
  ...
  // These exist only with Afantor_GFX (no subclass overrides)
  void
    ...
    drawCenterString(const char *string, int dX, int poY, int font),
    drawRightString(const char *string, int dX, int poY, int font),
    drawJpg(const uint8_t *jpg_data, size_t jpg_len, uint16_t x, uint16_t y),
...
class Afantor_GFX_Button {

 public:
  Afantor_GFX_Button(void);
  // "Classic" initButton() uses center & size
  void initButton(Afantor_GFX *gfx, int16_t x, int16_t y,
   uint16_t w, uint16_t h, uint16_t outline, uint16_t fill,
   uint16_t textcolor, char *label, uint8_t textsize);
  // New/alt initButton() uses upper-left corner & size
  void initButtonUL(Afantor_GFX *gfx, int16_t x1, int16_t y1,
   uint16_t w, uint16_t h, uint16_t outline, uint16_t fill,
   uint16_t textcolor, char *label, uint8_t textsize);
...
class GFXcanvas1 : public Afantor_GFX {
 public:
...
class GFXcanvas8 : public Afantor_GFX {
 public:
...
class GFXcanvas16 : public Afantor_GFX {
 public:
