/*!
 * @file Afantor_SPITFT.cpp
 *
 * @mainpage Afantor SPI TFT Displays (and some others)
 *
 * @section intro_sec Introduction
 *
 * Part of Afantor's GFX graphics library. Originally this class was
 * written to handle a range of color TFT displays connected via SPI,
 * but over time this library and some display-specific subclasses have
 * mutated to include some color OLEDs as well as parallel-interfaced
 * displays. The name's been kept for the sake of older code.
 *
 * Afantor invests time and resources providing this open source code,
 * please support Afantor and open-source hardware by purchasing
 * products from Afantor!

 * @section dependencies Dependencies
 *
 * This library depends on <a href="https://github.com/Afantor/Afantor_GFX">
 * Afantor_GFX</a> being present on your system. Please make sure you have
 * installed the latest version before using this library.
 *
 * @section author Author
 *
 * Written by Limor "ladyada" Fried for Afantor Industries,
...
#define digitalPinToPort(P) (&(PORT_IOBUS->Group[g_APinDescription[P].ulPort]))
 ...
 #define wrPeripheral PIO_CCL // Use CCL to invert write strobe
    ...
    static void dma_callback(Afantor_ZeroDMA *dma) {
        dma_busy = false;
    }
...
// Possible values for Afantor_SPITFT.connection:
...
/*!
    @brief   Afantor_SPITFT constructor for software (bitbang) SPI.
...
*/
Afantor_SPITFT::Afantor_SPITFT(uint16_t w, uint16_t h,
  int8_t cs, int8_t dc, int8_t mosi, int8_t sck, int8_t rst, int8_t miso) :
  Afantor_GFX(w, h), connection(TFT_SOFT_SPI), _rst(rst), _cs(cs), _dc(dc) {
    swspi._sck  = sck;
    swspi._mosi = mosi;
    swspi._miso = miso;
...
/*!
    @brief   Afantor_SPITFT constructor for hardware SPI using the board's
...
Afantor_SPITFT::Afantor_SPITFT(uint16_t w, uint16_t h, int8_t cs,
  int8_t dc, int8_t rst) : Afantor_GFX(w, h),
  connection(TFT_HARD_SPI), _rst(rst), _cs(cs), _dc(dc) {
    hwspi._spi = &SPI;
}
#else  // !ESP8266
Afantor_SPITFT::Afantor_SPITFT(uint16_t w, uint16_t h, int8_t cs,
  int8_t dc, int8_t rst) : Afantor_SPITFT(w, h, &SPI, cs, dc, rst) {
...
/*!
    @brief   Afantor_SPITFT constructor for hardware SPI using a specific
             SPI peripheral.
    ...
    @note    Output pins are not initialized; application typically will
             ...
             this library's initSPI() function to initialize pins.
*/
Afantor_SPITFT::Afantor_SPITFT(uint16_t w, uint16_t h, SPIClass *spiClass,
  int8_t cs, int8_t dc, int8_t rst) : Afantor_GFX(w, h),
  connection(TFT_HARD_SPI), _rst(rst), _cs(cs), _dc(dc) {
    hwspi._spi = spiClass;
#if defined(USE_FAST_PINIO)
 #if defined(HAS_PORT_SET_CLR)
...
/*!
    @brief   Afantor_SPITFT constructor for parallel display connection.
    @param   w         Display width in pixels at default rotation (0).
    @param   h         Display height in pixels at default rotation (0).
...
*/
Afantor_SPITFT::Afantor_SPITFT(uint16_t w, uint16_t h, tftBusWidth busWidth,
  int8_t d0, int8_t wr, int8_t dc, int8_t cs, int8_t rst, int8_t rd) :
  Afantor_GFX(w, h), connection(TFT_PARALLEL), _rst(rst), _cs(cs), _dc(dc) {
    tft8._d0  = d0;
    tft8._wr  = wr;
    tft8._rd  = rd;
...
*/
void Afantor_SPITFT::initSPI(uint32_t freq) {
...
*/
void Afantor_SPITFT::startWrite(void) {
...
/*!
    @brief  Call after issuing command(s) or data to display. Performs
            ...
            for all display types; not an SPI-specific function.
*/
void Afantor_SPITFT::endWrite(void) {
...
*/
void Afantor_SPITFT::writePixel(int16_t x, int16_t y, uint16_t color) {
...
*/
void Afantor_SPITFT::writePixels(uint16_t *colors, uint32_t len,
...
*/
void Afantor_SPITFT::dmaWait(void) {
#if defined(USE_SPI_DMA)
    while(dma_busy);
 #if defined(__SAMD51__)
    if(connection == TFT_HARD_SPI) {
...
*/
void Afantor_SPITFT::writeColor(uint16_t color, uint32_t len) {
...
*/
void Afantor_SPITFT::writeFillRect(int16_t x, int16_t y,
...
*/
void inline Afantor_SPITFT::writeFastHLine(int16_t x, int16_t y, int16_t w,
  uint16_t color) {
...
/*!
    ...
    @param  color  16-bit line color in '565' RGB format.
*/
void inline Afantor_SPITFT::writeFastVLine(int16_t x, int16_t y, int16_t h,
  uint16_t color) {
...
*/
inline void Afantor_SPITFT::writeFillRectPreclipped(int16_t x, int16_t y,
  int16_t w, int16_t h, uint16_t color) {
...
/*!
    ...
    @param  color  16-bit pixel color in '565' RGB format.
*/
void Afantor_SPITFT::drawPixel(int16_t x, int16_t y, uint16_t color) {
    // Clip first...
    if((x >= 0) && (x < _width) && (y >= 0) && (y < _height)) {
...
/*!
    ...
    @note   This repeats the writeFillRect() function almost in its entirety,
            ...
            that much code.
*/
void Afantor_SPITFT::fillRect(int16_t x, int16_t y, int16_t w, int16_t h,
  uint16_t color) {
    if(w && h) {                            // Nonzero width and height?
        if(w < 0) {                         // If negative width...
            x +=  w + 1;                    //   Move X to left edge
...
*/
void Afantor_SPITFT::drawFastHLine(int16_t x, int16_t y, int16_t w,
  uint16_t color) {
    if((y >= 0) && (y < _height) && w) { // Y on screen, nonzero width
        if(w < 0) {                      // If negative width...
            x +=  w + 1;                 //   Move X to left edge
...
/*!
    ...
    @note   This repeats the writeFastVLine() function almost in its
            ...
            transaction isn't performed at all if the line is rejected.
*/
void Afantor_SPITFT::drawFastVLine(int16_t x, int16_t y, int16_t h,
  uint16_t color) {
    if((x >= 0) && (x < _width) && h) { // X on screen, nonzero height
        if(h < 0) {                     // If negative height...
            y +=  h + 1;                //   Move Y to top edge
...
/*!
    ...
    @param  color  16-bit pixel color in '565' RGB format.
*/
void Afantor_SPITFT::pushColor(uint16_t color) {
    startWrite();
    SPI_WRITE16(color);
    endWrite();
}
...
*/
void Afantor_SPITFT::drawRGBBitmap(int16_t x, int16_t y,
...
*/
void Afantor_SPITFT::invertDisplay(bool i) {
    startWrite();
...
*/
uint16_t Afantor_SPITFT::color565(uint8_t red, uint8_t green, uint8_t blue) {
...
*/
inline void Afantor_SPITFT::SPI_BEGIN_TRANSACTION(void) {
    if(connection == TFT_HARD_SPI) {
#if defined(SPI_HAS_TRANSACTION)
...
*/
inline void Afantor_SPITFT::SPI_END_TRANSACTION(void) {
#if defined(SPI_HAS_TRANSACTION)
    if(connection == TFT_HARD_SPI) {
...
*/
void Afantor_SPITFT::spiWrite(uint8_t b) {
    if(connection == TFT_HARD_SPI) {
#if defined(__AVR__)
...
*/
void Afantor_SPITFT::writeCommand(uint8_t cmd) {
    SPI_DC_LOW();
    spiWrite(cmd);
    SPI_DC_HIGH();
}
...
*/
uint8_t Afantor_SPITFT::spiRead(void) {
    uint8_t  b = 0;
    uint16_t w = 0;
    if(connection == TFT_HARD_SPI) {
...
*/
inline void Afantor_SPITFT::SPI_MOSI_HIGH(void) {
#if defined(USE_FAST_PINIO)
 #if defined(HAS_PORT_SET_CLR)
  #if defined(KINETISK)
    *swspi.mosiPortSet = 1;
...
*/
inline void Afantor_SPITFT::SPI_MOSI_LOW(void) {
#if defined(USE_FAST_PINIO)
 #if defined(HAS_PORT_SET_CLR)
  #if defined(KINETISK)
    *swspi.mosiPortClr = 1;
...
*/
inline void Afantor_SPITFT::SPI_SCK_HIGH(void) {
#if defined(USE_FAST_PINIO)
 #if defined(HAS_PORT_SET_CLR)
  #if defined(KINETISK)
    *swspi.sckPortSet = 1;
...
*/
inline void Afantor_SPITFT::SPI_SCK_LOW(void) {
#if defined(USE_FAST_PINIO)
 #if defined(HAS_PORT_SET_CLR)
  #if defined(KINETISK)
    *swspi.sckPortClr = 1;
...
*/
inline bool Afantor_SPITFT::SPI_MISO_READ(void) {
#if defined(USE_FAST_PINIO)
 #if defined(KINETISK)
...
*/
void Afantor_SPITFT::SPI_WRITE16(uint16_t w) {
    if(connection == TFT_HARD_SPI) {
#if defined(__AVR__)
...
*/
void Afantor_SPITFT::SPI_WRITE32(uint32_t l) {
    if(connection == TFT_HARD_SPI) {
#if defined(__AVR__)
...
*/
inline void Afantor_SPITFT::TFT_WR_STROBE(void) {
#if defined(USE_FAST_PINIO)
 #if defined(HAS_PORT_SET_CLR)
  #if defined(KINETISK)
    *tft8.wrPortClr = 1;
...
/*!
    @brief  Set the RD line HIGH. Used for parallel-connected interfaces
            when reading data.
*/
inline void Afantor_SPITFT::TFT_RD_HIGH(void) {
#if defined(USE_FAST_PINIO)
 #if defined(HAS_PORT_SET_CLR)
...
/*!
    @brief  Set the RD line LOW. Used for parallel-connected interfaces
            when reading data.
*/
inline void Afantor_SPITFT::TFT_RD_LOW(void) {
#if defined(USE_FAST_PINIO)
 #if defined(HAS_PORT_SET_CLR)
